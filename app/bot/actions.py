...
async def _process_add_paper_submission(ack, body, client, logger, db, paper_service, user_service):
    ...
    if bibtex_str:
        try:
            ...
            if bib_database.entries:
                ...
                parsed_bibtex_data["authors"] = _parse_bibtex_authors(entry.get("author", ""))
                # Attempt to parse year for published_date
                if "year" in entry:
                    try:
                        # Use January 1st of the given year
                        parsed_bibtex_data["published_date"] = datetime(int(entry["year"]), 1, 1)
                ...
                parsed_bibtex_data["arxiv_id"] = entry.get("eprint") # Often found in eprint field for arXiv
            else:
    ...
    final_authors = authors_str if authors_str else parsed_bibtex_data.get("authors", [])
    final_published_date = published_date_str if published_date_str else (parsed_bibtex_data.get("published_date").strftime("%Y-%m-%d") if parsed_bibtex_data.get("published_date") else None)
    ...
    if not final_title or not final_url:
        await ack(
            response_action="errors",
            errors={"paper_title_block": "Either bibtex must be provided, or both title and url must be provided."},
        )
        return
    ...
    if final_published_date:
        try:
            ...
            await ack(
                response_action="errors",
                errors={"paper_published_date_block": "날짜 형식이 올바르지 않습니다. YYYY-MM-DD 형식으로 입력해주세요."},
            )
            return
...
def register_actions(app: AsyncApp):
    ...
    async def handle_add_paper_modal_submission(ack, body, client, logger):
        ...
        paper_service = PaperService(db)
        user_service = UserService(db)
        await _process_add_paper_submission(ack, body, client, logger, db, paper_service, user_service)
        db.close()
